...
namespace miniplc0 {
	...
	//输出文本文件
	void Analyser::print_text(std::ostream& output) {
		...
		for (int32_t i = 0; i < _instructions.size(); i++) {
			output << i << " ";
			printInstruction(_instructions[i], output);
		}
		...
		for (int32_t i = 0; i < _functions.size(); i++) {
			...
			for (int32_t j = 0; j < _functions[i].instructions.size(); j++) {
				output << j << " ";
				printInstruction(_functions[i].instructions[j], output);
			}
		}
	}

	//输出文本指令
	void Analyser::printInstruction(Instruction instruction, std::ostream& output) {
		std::string name;
